**Neu v0.28:** Der Datei-Upload bekommt BX3-schonendes Pacing nur im exklusiven Uploadpfad: 45 ms zwischen den 512-Byte-UART-Teilsegmenten und 140 ms vor der Blockpruefung. Zwischen-ACKs bleiben entfallen. Die Blockpruefung hat jetzt einen 5-s-Timeout mit bestehendem automatischem Block-Retry. Die Uploadseite merkt den BX3-Zielpfad lokal im Browser, bietet ein zusaetzliches Feld fuer einen einkopierten lokalen Pfad als Referenz und akzeptiert Dateien per Drag&Drop. Wegen Browser-Sicherheitsregeln kann ein lokaler Dateipfad allein nicht zum Lesen einer Datei verwendet werden.

**Neu v0.27:** Die UART-Konsole verwendet jetzt sequenzielles HTTP-Polling statt eines festen 150-ms-Intervalls. Dadurch kann nie mehr als eine `/uart/console/data`-Abfrage gleichzeitig laufen. Antworten, deren angeforderte Sequenz zwischenzeitlich veraltet ist, werden verworfen. Zusaetzliche Browser-Diagnosezaehler zeigen uebernommene RX-Bytes, Poll-Anzahl und verworfene veraltete Antworten an. Dies verhindert die beobachtete Mehrfachdarstellung identischer BX3-Ausgaben, waehrend Decoder und UART-Rohpuffer unveraendert bleiben.

**Neu v0.26:** Der Datei-Upload ist auf Durchsatz optimiert: logische Uploadbloecke wurden von 2048 auf 4096 Byte vergroessert, UART-Teilsegmente von 96 auf 512 Byte. Die Teilsegmente eines Blocks werden ohne einzelne `wc -c`/ACK-Roundtrips direkt nacheinander gesendet. Nach jedem kompletten 4096-Byte-Block bleiben Groessen- und POSIX-`cksum`-Pruefung sowie Block-Retry erhalten.

**Neu v0.25:** Der Datei-Upload wurde fuer die BX3-UART-Shell robuster gemacht. Ein logischer 2048-Byte-Uploadblock wird nicht mehr als eine mehrere Kilobyte lange Base64-Shellzeile gesendet, sondern in 96-Byte-UART-Teilsegmente zerlegt. Upload-Zielpfade sind auf 80 Zeichen begrenzt, damit auch die erzeugten Segmentbefehle kurz bleiben. Jedes Teilsegment wird von der BX3 mit einer kurzen Marker-Antwort bestaetigt; erst danach folgt die POSIX-`cksum`-Pruefung des vollstaendigen Blocks. Die Weboberflaeche zeigt Phase, Teilsegment, RX-Stille, Fehler und Retries. Die Vorbereitung meldet nun explizit Pfad-/Schreibfehler sowie fehlende bzw. inkompatible `base64 -d`/`cksum`-Werkzeuge statt nur in einen allgemeinen Timeout zu laufen.
