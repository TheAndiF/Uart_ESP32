...
**Thema:** Gemeinsamer UART-Basisbetrieb mit Autoscroll in der universellen UART Konsole  
...
**Referenzen:** `BX3_ESP32_Login_Leitfaden.pdf`; `Regeln_Projektdokumentation_PDF_DOCX_Pflicht_.pdf`; Projektstand v0.15
...
| v0.16 | 2026-09-20 | OpenAI ChatGPT | Entwurf | Häkchenfeld für Autoscroll in der UART Konsole ergänzt; Einstellung wird browserseitig gespeichert und beeinflusst den UART-Betrieb nicht. |
...
Seit v0.15 besteht eine klare Trennung zwischen der **physischen UART-Verbindung** und den darauf aufsetzenden Funktionen. v0.16 ergänzt diese Architektur ausschließlich um eine Anzeigeoption für die UART Konsole. Die vier eigenständigen Hauptbereiche bleiben:
...
WLAN, NTP, MQTT, Batterieüberwachung, Deep Sleep, OTA und die übrige bestehende Infrastruktur bleiben erhalten. Die Änderung v0.16 betrifft nur die Browserdarstellung der UART Konsole.
...
### 6.2 Autoscroll v0.16

Direkt im Terminalbereich befindet sich ab v0.16 das Häkchenfeld **Autoscroll bei neuen UART-Daten**.

- Beim ersten Aufruf ist Autoscroll standardmäßig aktiviert.
- Ist das Häkchen gesetzt, wird das Terminal nach jeder neuen Darstellung automatisch bis zum neuesten Datenende gescrollt.
- Ist das Häkchen entfernt, bleibt die aktuelle manuelle Scrollposition erhalten. Neue UART-Daten werden trotzdem weiterhin empfangen und im Browserpuffer gesammelt.
- Beim erneuten Aktivieren springt die Ansicht sofort zum Ende des Terminals.
- Der Zustand wird nur im Browser über `localStorage` unter dem Schlüssel `uartConsoleAutoscroll` gespeichert.
- Es werden dafür keine ESP32-NVS-Werte geändert und keine UART-Parameter neu gestartet oder gespeichert.

Damit kann während einer laufenden UART-Aufzeichnung in älteren Terminaldaten gelesen werden, ohne dass jede 150-ms-Aktualisierung die Ansicht wieder nach unten zieht.
...
Autoscroll benötigt keine zusätzliche Webroute. Die Einstellung wird vollständig im Browser verarbeitet.
...
| Datei | Änderung |
|---|---|
| `src/main.cpp` | Autoscroll-Häkchenfeld, browserseitige Speicherung und bedingtes Scrollen ergänzt; Build-Version auf v0.16 gesetzt. |
| `src/ConfigDefaults.h` | Firmwareversion auf `0.16.0` angehoben. |
| `README.md` | Bedienung und Verhalten von Autoscroll v0.16 dokumentiert. |
| `VALIDATION_v0.16.txt` | Validierungsstand und Prüfungen für v0.16 dokumentiert. |
| `docs/2026-09-20 - Uart_Esp32 - Projektdokumentation - Firmware - v0.16.*` | PDF-, DOCX- und Markdown-Dokumentation. |
| `docs/2026-09-20 - Uart_Esp32 - Commit-Nachricht - UART-Konsole-Autoscroll - v0.16.txt` | Vorgeschriebene englische Commit-Nachricht. |

## 14. Validierung

Folgende Prüfungen wurden lokal erfolgreich durchgeführt:

1. C++17-Syntaxprüfung von `main.cpp` gegen die vorhandenen lokalen Arduino-/ESP32-Framework-Stubs.
2. Statische Prüfung, dass das Häkchenfeld `autoscroll` im Terminal vorhanden und standardmäßig aktiviert ist.
3. JavaScript-Syntaxprüfung des extrahierten UART-Konsolen-Skripts mit Node.js.
4. Statische Prüfung, dass `render()` nur bei aktiviertem Autoscroll `terminal.scrollTop = terminal.scrollHeight` ausführt.
5. Statische Prüfung der Browser-Persistenz über `localStorage` mit dem Schlüssel `uartConsoleAutoscroll`.
6. Prüfung, dass UART Einstellungen, UART Decoder, UART Probe-Runner sowie WLAN, MQTT, Batterie, Deep Sleep und OTA im Quelltext weiterhin vorhanden sind.
7. Vergleich gegen v0.15: An der UART-Empfangs-, Decoder-, Probe-Runner- und TX-Gating-Logik in `UartManager.*` wurden keine Änderungen vorgenommen.

**Nicht durchgeführt:** In der Arbeitsumgebung ist PlatformIO nicht installiert. Deshalb konnte kein echter ESP32-Arduino-Framework-Build erzeugt und keine Firmware auf Hardware geflasht werden. Ebenso wurde kein realer Browser gegen einen laufenden ESP32 verwendet. Die Autoscroll-Funktion ist browserseitig implementiert und wurde hier per Quelltext-, JavaScript-Syntax- und Dokumentationsprüfung validiert.
...
## 16. Inbetriebnahme-Checkliste v0.16
...
- [ ] In UART Konsole prüfen, dass **Autoscroll** standardmäßig aktiviert ist und neuen Daten folgt.
- [ ] Autoscroll deaktivieren, nach oben scrollen und prüfen, dass neue RX-Daten die Scrollposition nicht verändern.
- [ ] Autoscroll wieder aktivieren und prüfen, dass die Ansicht sofort zum neuesten Datenende springt.
...
3. Projektquellstand **Uart_Esp32 v0.15** als technische Ausgangsbasis der Firmwareänderung.
