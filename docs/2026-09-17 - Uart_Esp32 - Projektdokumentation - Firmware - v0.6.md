...
| v0.6 | 2026-09-17 | OpenAI ChatGPT | Entwurf | Compilerfehler in markWifiConnected() behoben; Versionsstand aktualisiert |

## Ziel des Standes v0.6

Der Stand v0.6 behebt einen C++-Syntaxfehler in `markWifiConnected()`, der den lokalen PlatformIO-Build mit `else without a previous if` abbrach. Ursache war eine Zuweisung zwischen einem `if` und dem zugehoerigen `else if`. Die WLAN-Fallback-Architektur aus v0.5 bleibt unveraendert erhalten.
...
Uart_Esp32/
  ...
  src/
    ...
    main.cpp
  docs/
    2026-09-17 - Uart_Esp32 - Projektdokumentation - Firmware - v0.6.*
    2026-09-17 - Uart_Esp32 - Commit-Nachricht - Buildfix-markWifiConnected - v0.6.txt
```
...
- `docs/2026-09-17 - Uart_Esp32 - Commit-Nachricht - Buildfix-markWifiConnected - v0.6.txt`
...
Bereitgestellt werden ein vollstaendiges ZIP mit Root-Ordner `Uart_Esp32`, ein ZIP mit nur geaenderten/neuen Dateien in derselben Struktur, eine Patch-Datei, eine englische Commit-Nachricht mit Datum und Dateiliste sowie Projektdokumentation als PDF und bearbeitbare DOCX mit identischem Stand v0.6.
