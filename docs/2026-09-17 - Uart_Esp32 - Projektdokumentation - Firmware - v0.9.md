...
**Thema:** Firmware fuer ESP32-WROOM-32 / NodeMCU-32S - UART-Routingkorrektur, getrennte Einstellungen und Decoder-Dauerbetrieb  
...
**Referenzen:** `Regeln_Projektdokumentation_PDF_DOCX_Pflicht_(5).pdf`; `ESP32_UART_Protokoll_Entschluesselung(1).pdf`; Projektstand v0.8
...
## Ziel des Standes v0.9

Der Projektstand v0.9 korrigiert einen Integrationsfehler der UART-Weboberflaeche. Bei der verwendeten ESPAsyncWebServer-Version konnte die zuerst registrierte kurze Route `/uart` auch Anfragen an die Unterrouten `/uart/settings` und `/uart/status` abfangen. Das erklaert das beobachtete Verhalten: Im Browser stand zwar `/uart/settings`, angezeigt wurde aber erneut die Monitorseite. Gleichzeitig erhielt das JavaScript fuer `/uart/status` HTML statt JSON; dadurch blieben Byte- und Paketzaehler sichtbar auf 0, obwohl im Raw-Puffer bereits UART-Daten ankamen.

Ab v0.9 werden die spezifischen Routen zuerst registriert. Die Live-Seite besitzt nun die eindeutige Route `/uart/monitor`; `/uart` ist nur noch ein kompatibler Redirect. Die Status-API wird vor allen anderen UART-Routen registriert und liefert explizit JSON mit `Cache-Control: no-store`. Die Monitorseite prueft den Content-Type und zeigt den Zustand der Status-API sichtbar an.

Der Decoder selbst bleibt ein echter Dauerbetrieb: Nach Aktivierung wird er in der ESP32-Hauptschleife kontinuierlich ausgefuehrt, unabhaengig davon, ob eine Browserseite geoeffnet ist. Der gewaehlte Modus bleibt in NVS gespeichert und wird nach einem Neustart automatisch wieder gestartet.
...
Ab v0.9 sind Konfiguration und Beobachtung bewusst getrennt. Dadurch bleibt die Live-Seite uebersichtlich, waehrend die umfangreichen Einstellungen auf einer eigenen Seite liegen.
...
| `/uart/monitor` | GET | Reine UART-Liveansicht / Decoder-Monitor |
...
Die Korrektur in v0.9 besteht aus vier Massnahmen:
...
Uart_ESP32/
  ...
  src/
    ...
    main.cpp
  docs/
    2026-09-17 - Uart_Esp32 - Projektdokumentation - Firmware - v0.9.*
    2026-09-17 - Uart_Esp32 - Commit-Nachricht - UART-Seitentrennung-und-Dauerbetrieb - v0.9.txt
```
...
## Geaenderte / neue Dateien in v0.9

- `README.md`
- `src/ConfigDefaults.h`
- `src/main.cpp`
- `docs/2026-09-17 - Uart_Esp32 - Projektdokumentation - Firmware - v0.9.md` (neu)
- `docs/2026-09-17 - Uart_Esp32 - Projektdokumentation - Firmware - v0.9.docx` (neu)
- `docs/2026-09-17 - Uart_Esp32 - Projektdokumentation - Firmware - v0.9.pdf` (neu)
- `docs/2026-09-17 - Uart_Esp32 - Commit-Nachricht - UART-Routing-und-Live-Status - v0.9.txt` (neu)
