...
static String uartConsolePage() {
  ...
  h += uartNav(); h += "<a class='btn' href='/'>Zurueck</a>";
  h += R"rawliteral(<script>
let consoleSeq=0,bytes=[],truncNote=false;let consoleTxReady=false,consoleSending=false,lastSendAt=0,lastSendKey='';const SEND_COOLDOWN_MS=850;const terminal=document.getElementById('terminal'),input=document.getElementById('console_input'),api=document.getElementById('console_api'),autoscroll=document.getElementById('autoscroll'),pauseDisplay=document.getElementById('pause_display');
