...
static const char* FW_BUILD_VERSION = "v0.29";
...
static String pageHead(const String& title) {
  String h = F("<!doctype html><html><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><meta http-equiv='Cache-Control' content='no-store, no-cache, must-revalidate'><meta http-equiv='Pragma' content='no-cache'><meta http-equiv='Expires' content='0'>");
...
static String uartConsolePage() {
  ...
  h += "<p class='small'><b>Browser-Polling:</b> RX uebernommen <span id='con_browser_rx'>0</span> Bytes, Polls <span id='con_poll_count'>0</span>, verworfene veraltete Antworten <span id='con_stale_polls'>0</span>, aktiv <span id='con_poll_active'>nein</span>.</p>";
  ...
  h += "<p class='small'><b>Puffer:</b> 8192 RX-Bytes, sequenzielle inkrementelle Abfrage mit 150 ms Pause nach jeder abgeschlossenen HTTP-Antwort. Es laeuft hoechstens eine Konsolenabfrage gleichzeitig; veraltete Antworten werden verworfen. Bei sehr hohen kontinuierlichen Baudraten kann HTTP-Polling weiterhin Daten verlieren; der Decoder und Rohpuffer auf dem ESP32 arbeiten davon unabhaengig. BX3-Arbeitswert laut Leitfaden: 115200/8N1.</p>";
  h += uartNav(); h += "<a class='btn' href='/'>Zurueck</a>";
  h += R"rawliteral(<script>
let consoleSeq=0,bytes=[],truncNote=false;let consoleTxReady=false,consoleSending=false,lastSendAt=0,lastSendKey='';let consolePollActive=false,consolePollStopped=false,consolePollCount=0,consoleStalePolls=0,consoleBrowserRxBytes=0;const SEND_COOLDOWN_MS=850;const terminal=document.getElementById('terminal'),input=document.getElementById('console_input'),api=document.getElementById('console_api'),autoscroll=document.getElementById('autoscroll'),pauseDisplay=document.getElementById('pause_display');
...
function setTxButtons(ok){consoleTxReady=!!ok;const disabled=!consoleTxReady||consoleSending;for(const id of ['send_btn','enter_btn','ctrlc_btn','ctrld_btn','tab_btn','esc_btn'])document.getElementById(id).disabled=disabled;}
function beginConsoleSend(kind,key){const now=Date.now();if(consoleSending){api.textContent='Senden laeuft bereits - bitte warten.';return false;}if(now-lastSendAt<SEND_COOLDOWN_MS&&key===lastSendKey){api.textContent='Doppelte Eingabe blockiert.';return false;}consoleSending=true;lastSendAt=now;lastSendKey=key;setTxButtons(consoleTxReady);return true;}
function endConsoleSend(){consoleSending=false;setTxButtons(consoleTxReady);}
async function pollConsole(){if(consolePollActive||consolePollStopped)return;consolePollActive=true;document.getElementById('con_poll_active').textContent='ja';const requestedSeq=consoleSeq;try{consolePollCount++;const r=await fetch('/uart/console/data?since='+requestedSeq,{cache:'no-store'});if(!r.ok)throw new Error('HTTP '+r.status);const d=await r.json();if(consoleSeq!==requestedSeq){consoleStalePolls++;return;}if(consoleSeq===0&&terminal.textContent.startsWith('Verbinde'))terminal.textContent='';if(d.truncated){bytes=[];truncNote=true;}const hex=d.hex||'';ingestHex(hex);consoleBrowserRxBytes+=Math.floor(hex.length/2);consoleSeq=Number(d.sequence)||0;document.getElementById('con_running').textContent=d.running?'laeuft':'gestoppt';document.getElementById('con_tx_enabled').textContent=d.tx_enabled?'ja':'nein';document.getElementById('con_rx').textContent=d.rx_bytes;document.getElementById('con_tx').textContent=d.tx_bytes;document.getElementById('con_tx_owner').textContent=d.tx_owner;setTxButtons(!!d.tx_ready);api.textContent=d.tx_ready?'Live - TX bereit':'Live - TX nicht bereit: '+d.tx_owner;render();}catch(e){api.textContent='FEHLER: '+e;}finally{consolePollActive=false;document.getElementById('con_poll_active').textContent='nein';document.getElementById('con_browser_rx').textContent=consoleBrowserRxBytes;document.getElementById('con_poll_count').textContent=consolePollCount;document.getElementById('con_stale_polls').textContent=consoleStalePolls;if(!consolePollStopped)setTimeout(pollConsole,150);}}
async function sendForm(data,fmt){const key=fmt+':'+data;if(!beginConsoleSend('data',key))return false;try{const expected=(fmt==='hex')?Math.ceil((data.match(/[0-9a-fA-F]{2}/g)||[]).length):data.length;const r=await fetch('/uart/console/send',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8'},body:'format='+encodeURIComponent(fmt)+'&data='+encodeURIComponent(data)});if(!r.ok){api.textContent='SENDEN FEHLER: '+await r.text();return false;}api.textContent='gesendet: '+expected+' Byte';return true;}catch(e){api.textContent='SENDEN FEHLER: '+e;return false;}finally{endConsoleSend();}}
async function sendLine(){const m=document.getElementById('input_mode').value;let payload='',fmt='text';if(m==='hex'){fmt='hex';payload=input.value.trim();const e=endingHex();if(e)payload+=(payload?' ':'')+e;}else{const e=endingHex(),suffix=e==='0D'?'\r':e==='0A'?'\n':e==='0D0A'?'\r\n':'';payload=input.value+suffix;}if(payload.length===0){api.textContent='Keine Eingabe.';input.focus();return;}const original=input.value;input.value='';const ok=await sendForm(payload,fmt);if(!ok&&input.value==='')input.value=original;input.focus();}
...
static String uartImageTransferPage() {
  ...
  h += "<p class='small'>Verifiziertes, blockweises Backup eines BX3-MTD-Devices oder einer vorbereiteten Snapshot-Datei ueber die serielle Shell. v0.25 behaelt diese Richtung als Datei-Download; die bewaehrte v0.23-Transferlogik bleibt intern kompatibel. CRC32 (POSIX cksum) prueft jeden Block; die Gesamt-SHA256-Pruefung bleibt erhalten.</p>";
...
const SOURCE_DEFAULT='/tmp/mtd7.img.gz';
const SOURCE_STORE='uartImageSourceV23';
function validSavedSource(v){return typeof v==='string'&&v.length>0&&v.length<80;}
function loadImageSource(){try{const v=localStorage.getItem(SOURCE_STORE);return validSavedSource(v)?v:SOURCE_DEFAULT;}catch(e){return SOURCE_DEFAULT;}}
function saveImageSource(){const v=(imgSource.value||'').trim();try{localStorage.setItem(SOURCE_STORE,v);localStorage.setItem('uartImageSource',v);}catch(e){}const n=document.getElementById('img_next_src');if(n)n.textContent=v||'-';}
function setImageSource(v){imgSource.value=v;saveImageSource();imgSource.focus();imgSource.select();}
function getImageSource(){const v=(imgSource.value||'').trim();imgSource.value=v;saveImageSource();return v;}
imgSource.value=loadImageSource();saveImageSource();
document.getElementById('src_snapshot').addEventListener('click',()=>setImageSource('/tmp/mtd7.img.gz'));document.getElementById('src_live').addEventListener('click',()=>setImageSource('/dev/mtd7ro'));document.getElementById('src_clear').addEventListener('click',()=>setImageSource(''));
imgSource.addEventListener('input',saveImageSource);imgSource.addEventListener('change',saveImageSource);imgSource.addEventListener('blur',saveImageSource);
async function post(url,body=''){return fetch(url,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8','Cache-Control':'no-store'},cache:'no-store',body});}
function resetBrowserImage(){chunks=[];pendingBlock=-1;pendingBytes=null;if(downloadUrl){URL.revokeObjectURL(downloadUrl);downloadUrl=null;}document.getElementById('img_download').style.display='none';}
async function startImage(){resetBrowserImage();startedAtBlock=Number(document.getElementById('img_start').value)||0;const src=getImageSource();if(!src){document.getElementById('img_note').textContent='Quelle fehlt.';return;}const r=await post('/uart/image/start','source='+encodeURIComponent(src)+'&start='+startedAtBlock);if(!r.ok){document.getElementById('img_status').textContent=await r.text();return;}document.getElementById('img_note').textContent='Transfer gestartet.';pollImage();}
async function restartImage(){resetBrowserImage();startedAtBlock=0;document.getElementById('img_start').value='0';const src=getImageSource();if(!src){document.getElementById('img_note').textContent='Quelle fehlt.';return;}const r=await post('/uart/image/restart','source='+encodeURIComponent(src));if(!r.ok){document.getElementById('img_note').textContent='Neustart gesperrt: '+await r.text();return;}document.getElementById('img_note').textContent='Transfer nach Konsolenbestaetigung neu gestartet.';pollImage();}
async function abortImage(){await post('/uart/image/abort');document.getElementById('img_note').textContent='Transfer abgebrochen. UART Konsole wird freigegeben.';}
async function ackPending(){if(pendingBlock<0||!pendingBytes)return false;const a=await post('/uart/image/ack','block='+pendingBlock);if(!a.ok)throw new Error(await a.text());chunks.push(pendingBytes);pendingBlock=-1;pendingBytes=null;return true;}
async function fetchReadyBlock(d){if(busy)return;busy=true;try{if(pendingBytes){await ackPending();return;}if(!d.block_ready||Number(d.ready_block)<0)return;const block=Number(d.ready_block);const r=await fetch('/uart/image/block?block='+block,{cache:'no-store'});if(r.status===202){document.getElementById('img_note').textContent='Warte auf validierten Block '+block+' ...';return;}if(!r.ok)throw new Error(await r.text());const aBuf=new Uint8Array(await r.arrayBuffer());if(aBuf.length===0)throw new Error('Leerer Block');pendingBlock=block;pendingBytes=aBuf;await ackPending();document.getElementById('img_note').textContent='Block '+block+' uebernommen; warte auf naechsten validierten Block.';}catch(e){document.getElementById('img_note').textContent='Browser-/ACK-Fehler: '+e+' - Block bleibt fuer erneuten ACK erhalten.';}finally{busy=false;}}
function downloadName(src){src=src||'/dev/mtd7ro';const m=src.match(/\/dev\/mtd(\d+)ro$/);if(m)return 'bx3-mtd'+m[1]+'.img';let n=src.split('/').pop()||'bx3-image.bin';n=n.replace(/[^A-Za-z0-9._-]/g,'_');return n;}
...
static String uartFileUploadPage() {
  ...
  h += "<p class='small'>Beliebige lokale Dateien werden in logischen 4096-Byte-Bloecken ueber den ESP32 und die BX3-Shell nach /tmp oder /var/tmp uebertragen. v0.29 zerlegt jeden Block in 512-Byte-UART-Teilsegmente. Zwischen den Segmenten wird nur im Datei-Upload kurz gepaced; Zwischen-ACKs bleiben entfallen. Vor der Blockpruefung gibt es eine kurze Entlastungspause. Erst danach werden Groesse und POSIX cksum des kompletten Blocks geprueft. Die Zieldatei entsteht zuerst als .part und wird erst nach erfolgreicher Gesamtpruefung umbenannt.</p>";
  ...
  h += "<fieldset id='up_drop'><legend>Datei und Ziel</legend><label>Lokale Datei</label><input id='up_file' type='file'><p class='small'>Die Datei kann auch auf diesen Bereich gezogen werden. Ein lokaler Dateipfad kann aus Browser-Sicherheitsgruenden nicht allein per Text geladen werden.</p><label>Lokaler Dateipfad / Referenz (optional direkt einkopieren)</label><input id='up_local_path' placeholder='z. B. C:\\build\\ptybridge oder /home/user/build/ptybridge' maxlength='240' autocomplete='off' autocapitalize='off' spellcheck='false'><p class='small'>Dieses Feld dient als Referenz/Kopierhilfe. Fuer den eigentlichen Dateiinhalt muss die Datei ausgewaehlt oder per Drag&amp;Drop uebergeben werden.</p><label>BX3-Uploadpfad (direkt einkopieren)</label><input id='up_target' value='/tmp/bx3test/ptybridge' maxlength='80' autocomplete='off' autocapitalize='off' spellcheck='false'><label><input id='up_exec' type='checkbox' checked> Nach erfolgreicher Pruefung ausfuehrbar machen (chmod +x)</label><div class='row'><button id='up_start' type='button' onclick='startUpload()'>Upload starten</button><button id='up_abort' type='button' onclick='abortUpload()' disabled>Abbrechen</button></div><p class='small'>Erlaubt sind sichere Zielpfade unter /tmp/ oder /var/tmp/. Leerzeichen und Shell-Sonderzeichen sind absichtlich nicht erlaubt.</p></fieldset>";
  h += "<fieldset><legend>Status und Diagnose</legend><p><b>Status:</b> <span id='up_status'>bereit</span></p><p><b>Phase:</b> <span id='up_phase'>idle</span> &nbsp; <b>UART RX-Stille:</b> <span id='up_rxage'>0</span> ms</p><p><b>Ziel:</b> <span class='mono' id='up_target_status'>-</span></p><p><b>Datei:</b> <span id='up_name'>-</span></p><p><b>Fortschritt:</b> <span id='up_bytes'>0 / 0</span> Byte &nbsp; <b>Block:</b> <span id='up_block'>-</span></p><p><b>UART-Teilsegment:</b> <span id='up_segment'>-</span> &nbsp; <b>Segmentgroesse:</b> <span id='up_seglen'>-</span> Byte</p><p><b>CRC lokal:</b> <span id='up_lcrc'>-</span> &nbsp; <b>CRC BX3:</b> <span id='up_rcrc'>-</span></p><p><b>Fehler:</b> <span id='up_errors'>0</span> &nbsp; <b>Block-Retries:</b> <span id='up_retries'>0</span> &nbsp; <b>HTTP-Retries:</b> <span id='up_http_retries'>0</span> &nbsp; <b>Geschwindigkeit:</b> <span id='up_rate'>0</span> KiB/s</p><progress id='up_progress' value='0' max='100' style='width:100%;height:24px'></progress><p id='up_note' class='small'>Datei auswaehlen und Upload starten.</p></fieldset>";
  h += uartNav(); h += "<a class='btn' href='/'>Zurueck</a>";
  h += R"rawliteral(<script>
const UP_BLOCK=4096;
const UP_TARGET_STORE='uartUploadTargetV28';
const UP_LOCAL_REF_STORE='uartUploadLocalRefV28';
let uploadRunning=false,uploadFile=null,uploadOffset=0;
function loadUploadFields(){try{const t=localStorage.getItem(UP_TARGET_STORE);if(t)document.getElementById('up_target').value=t;const p=localStorage.getItem(UP_LOCAL_REF_STORE);if(p)document.getElementById('up_local_path').value=p;}catch(e){}}
function saveUploadFields(){try{localStorage.setItem(UP_TARGET_STORE,(document.getElementById('up_target').value||'').trim());localStorage.setItem(UP_LOCAL_REF_STORE,(document.getElementById('up_local_path').value||'').trim());}catch(e){}}
function setUploadFile(f){if(!f)return;uploadFile=f;document.getElementById('up_name').textContent=f.name+' ('+f.size+' Byte)';if(!document.getElementById('up_local_path').value.trim())document.getElementById('up_local_path').value=f.name;saveUploadFields();}

const UP_HTTP_RETRIES=3;
const UP_HTTP_RETRY_DELAY_MS=750;
let uploadHttpRetries=0;
function sleepUp(ms){return new Promise(r=>setTimeout(r,ms));}
function showHttpRetry(url,attempt){uploadHttpRetries++;const el=document.getElementById('up_http_retries');if(el)el.textContent=uploadHttpRetries;document.getElementById('up_note').textContent='HTTP-Verbindung unterbrochen; Wiederholung '+attempt+'/'+UP_HTTP_RETRIES+' fuer '+url+' ...';}
async function fetchUp(url,opts={},retry=true){let lastErr=null;for(let attempt=0;;attempt++){try{const r=await fetch(url,opts);if(retry&&r.status>=500&&attempt<UP_HTTP_RETRIES){showHttpRetry(url,attempt+1);await sleepUp(UP_HTTP_RETRY_DELAY_MS);continue;}return r;}catch(e){lastErr=e;if(!retry||attempt>=UP_HTTP_RETRIES)throw e;showHttpRetry(url,attempt+1);await sleepUp(UP_HTTP_RETRY_DELAY_MS);}}}
async function upPost(url,body='',retry=true){return fetchUp(url,{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded;charset=UTF-8','Cache-Control':'no-store'},cache:'no-store',body},retry);}
function bytesToB64(bytes){let s='';for(let i=0;i<bytes.length;i++)s+=String.fromCharCode(bytes[i]);return btoa(s);}
async function getUpStatus(){const r=await fetchUp('/uart/upload/status',{cache:'no-store'},true);if(!r.ok)throw new Error(await r.text());return r.json();}
function renderUp(d){document.getElementById('up_status').textContent=d.status||'-';document.getElementById('up_phase').textContent=d.phase||'-';document.getElementById('up_rxage').textContent=Number(d.last_rx_age_ms)||0;document.getElementById('up_target_status').textContent=d.target||'-';document.getElementById('up_bytes').textContent=d.accepted_bytes+' / '+d.total_size;document.getElementById('up_lcrc').textContent=Number(d.local_crc)?d.local_crc:'-';document.getElementById('up_rcrc').textContent=Number(d.remote_crc)?d.remote_crc:'-';document.getElementById('up_errors').textContent=Number(d.errors)||0;document.getElementById('up_retries').textContent=Number(d.chunk_retries)||0;document.getElementById('up_http_retries').textContent=uploadHttpRetries;document.getElementById('up_rate').textContent=(Number(d.rate_bps)/1024).toFixed(2);const pct=Number(d.total_size)?Math.min(100,Number(d.accepted_bytes)*100/Number(d.total_size)):0;document.getElementById('up_progress').value=pct;document.getElementById('up_abort').disabled=!d.active;document.getElementById('up_start').disabled=!!d.active||uploadRunning;document.getElementById('up_block').textContent=d.total_size?Math.min(Math.ceil(Number(d.accepted_bytes)/UP_BLOCK)+1,Math.ceil(Number(d.total_size)/UP_BLOCK))+' / '+Math.ceil(Number(d.total_size)/UP_BLOCK):'-';document.getElementById('up_segment').textContent=Number(d.segment_count)?(Math.min(Number(d.segment_index)+1,Number(d.segment_count))+' / '+d.segment_count+' (Offset '+d.segment_offset+')'):'-';document.getElementById('up_seglen').textContent=Number(d.segment_length)||'-';}
async function waitFor(pred,timeoutMs=20000){const start=Date.now();for(;;){const d=await getUpStatus();renderUp(d);if(d.error)throw new Error(d.status||'Uploadfehler');if(pred(d))return d;if(Date.now()-start>timeoutMs)throw new Error('Timeout beim Warten auf BX3-Bestaetigung');await sleepUp(150);}}
async function ensureUploadStarted(target,size,exec){for(let attempt=0;attempt<=UP_HTTP_RETRIES;attempt++){try{const r=await upPost('/uart/upload/start','target='+encodeURIComponent(target)+'&size='+size+'&exec='+exec,false);if(r.ok)return;const body=await r.text();if(r.status!==409)throw new Error(body||('HTTP '+r.status));}catch(e){if(attempt>=UP_HTTP_RETRIES)throw e;showHttpRetry('/uart/upload/start',attempt+1);}try{const d=await getUpStatus();renderUp(d);if(Number(d.total_size)===Number(size)&&d.target===target&&d.phase!=='idle'&&!d.error)return;if(d.error)throw new Error(d.status||'Uploadstart fehlgeschlagen');}catch(e){if(attempt>=UP_HTTP_RETRIES)throw e;}await sleepUp(UP_HTTP_RETRY_DELAY_MS);}throw new Error('Uploadstart konnte nicht bestaetigt werden');}
async function sendChunkResilient(offset,end,b64){for(let attempt=0;attempt<6;attempt++){let needPost=true;try{const s=await getUpStatus();renderUp(s);const accepted=Number(s.accepted_bytes);if(accepted===end&&s.phase==='ready')return true;if(accepted>offset&&accepted!==end)throw new Error('Upload-Offset unerwartet: '+accepted+' statt '+offset+' oder '+end);if(accepted===offset&&s.phase==='chunk_verify')needPost=false;if(s.retry_requested&&accepted===offset&&s.phase==='ready')needPost=true;}catch(e){/* Status-Fetch hat eigene Retries; bei Fehler folgt POST-Versuch. */}
if(needPost){try{const r=await upPost('/uart/upload/chunk','offset='+offset+'&data='+encodeURIComponent(b64),false);if(!r.ok&&r.status!==409)throw new Error(await r.text());}catch(e){showHttpRetry('/uart/upload/chunk',Math.min(attempt+1,UP_HTTP_RETRIES));}}
try{const d=await waitFor(d=>(Number(d.accepted_bytes)===end&&d.phase==='ready')||(d.retry_requested&&Number(d.accepted_bytes)===offset&&d.phase==='ready'),15000);if(Number(d.accepted_bytes)===end)return true;}catch(e){if(attempt>=5)throw e;}
document.getElementById('up_note').textContent='Block nicht bestaetigt; sicherer Wiederholungsversuch '+(attempt+1)+'/6 ab Offset '+offset+'.';await sleepUp(UP_HTTP_RETRY_DELAY_MS);}return false;}
async function ensureFinish(){for(let attempt=0;attempt<=UP_HTTP_RETRIES;attempt++){try{const s=await getUpStatus();renderUp(s);if(s.done)return s;if(s.phase==='verify'){return await waitFor(d=>d.done,30000);}if(s.phase!=='ready'||Number(s.accepted_bytes)!==Number(s.total_size))throw new Error('Upload noch nicht bereit zum Abschluss');const r=await upPost('/uart/upload/finish','',false);if(!r.ok&&r.status!==409)throw new Error(await r.text());return await waitFor(d=>d.done,30000);}catch(e){if(attempt>=UP_HTTP_RETRIES)throw e;showHttpRetry('/uart/upload/finish',attempt+1);await sleepUp(UP_HTTP_RETRY_DELAY_MS);}}}
async function startUpload(){if(uploadRunning)return;const f=document.getElementById('up_file').files[0]||uploadFile;saveUploadFields();const target=(document.getElementById('up_target').value||'').trim();if(!f){document.getElementById('up_note').textContent='Bitte zuerst eine Datei auswaehlen.';return;}if(!target){document.getElementById('up_note').textContent='BX3-Ziel fehlt.';return;}uploadRunning=true;uploadHttpRetries=0;document.getElementById('up_http_retries').textContent='0';uploadFile=f;uploadOffset=0;document.getElementById('up_name').textContent=f.name+' ('+f.size+' Byte)';document.getElementById('up_start').disabled=true;try{const exec=document.getElementById('up_exec').checked?'1':'0';await ensureUploadStarted(target,f.size,exec);await waitFor(d=>d.phase==='ready');while(uploadOffset<f.size){const end=Math.min(uploadOffset+UP_BLOCK,f.size);const bytes=new Uint8Array(await f.slice(uploadOffset,end).arrayBuffer());const b64=bytesToB64(bytes);const sent=await sendChunkResilient(uploadOffset,end,b64);if(!sent)throw new Error('Retry-Limit fuer Uploadblock erreicht');uploadOffset=end;document.getElementById('up_note').textContent='Block bestaetigt: '+uploadOffset+' / '+f.size+' Byte.';}const d=await ensureFinish();renderUp(d);document.getElementById('up_note').textContent='Upload erfolgreich verifiziert. Datei liegt auf der BX3 unter '+d.target+'.';}catch(e){document.getElementById('up_note').textContent='FEHLER: '+e;}finally{uploadRunning=false;try{renderUp(await getUpStatus());}catch(e){document.getElementById('up_start').disabled=false;}}}
...
document.getElementById('up_target').addEventListener('change',saveUploadFields);
document.getElementById('up_local_path').addEventListener('change',saveUploadFields);
document.getElementById('up_file').addEventListener('change',e=>setUploadFile(e.target.files&&e.target.files[0]));
const upDrop=document.getElementById('up_drop');
upDrop.addEventListener('dragover',e=>{e.preventDefault();});
upDrop.addEventListener('drop',e=>{e.preventDefault();const f=e.dataTransfer&&e.dataTransfer.files&&e.dataTransfer.files[0];if(f)setUploadFile(f);});
loadUploadFields();
...
static void registerRoutes() {
  ...
  // Browser -> BX3 file upload. The browser submits only one 4096-byte block
  ...
  server.on("/uart/image/block", HTTP_GET, [](AsyncWebServerRequest* r){
    const uint32_t requested = r->hasArg("block") ? (uint32_t)strtoul(r->arg("block").c_str(), nullptr, 10) : 0xFFFFFFFFUL;
    if (!uartMonitor.imageBlockReady() || requested == 0xFFFFFFFFUL || requested != uartMonitor.imageReadyBlock()) {
      AsyncWebServerResponse* wait = r->beginResponse(202, "application/json; charset=utf-8", String("{\"ready\":false,\"requested\":") + String(requested) + ",\"ready_block\":" + String(uartMonitor.imageBlockReady() ? (long)uartMonitor.imageReadyBlock() : -1L) + "}");
      wait->addHeader("Cache-Control", "no-store, no-cache, must-revalidate"); r->send(wait); return;
    }
    const size_t blockLength = uartMonitor.imageReadyBlockLength();
    AsyncWebServerResponse* resp = r->beginResponse("application/octet-stream", blockLength, [requested](uint8_t* buffer, size_t maxLen, size_t index) -> size_t {
      return uartMonitor.imageReadBlock(requested, index, buffer, maxLen);
    });
    resp->addHeader("X-UART-Image-Block", String(requested));
    resp->addHeader("X-UART-Image-Length", String(blockLength));
    resp->addHeader("Cache-Control", "no-store, no-cache, must-revalidate"); r->send(resp);
  });
  ...
  server.on("/uart/image/ack", HTTP_POST, [](AsyncWebServerRequest* r){
    if (!r->hasArg("block")) { r->send(400, "text/plain; charset=utf-8", "Blocknummer fehlt."); return; }
    const uint32_t block = (uint32_t)strtoul(r->arg("block").c_str(), nullptr, 10);
    if (!uartMonitor.imageAcknowledgeBlock(block)) { r->send(409, "text/plain; charset=utf-8", "ACK passt nicht zum browserbereiten Block."); return; }
    r->send(200, "text/plain; charset=utf-8", "OK");
  });
  server.on("/uart/image/console-confirm", HTTP_POST, [](AsyncWebServerRequest* r){
    if (!uartMonitor.confirmImageConsoleAccess()) { r->send(409, "text/plain; charset=utf-8", "Konsolenzugriff kann aktuell nicht bestaetigt werden. UART muss laufen und TX freigegeben sein."); return; }
    r->send(200, "text/plain; charset=utf-8", "OK");
  });
  server.on("/uart/image/restart", HTTP_POST, [](AsyncWebServerRequest* r){
    if (!uartMonitor.imageRestartAllowed()) { r->send(409, "text/plain; charset=utf-8", "Neustart gesperrt: zuerst Konsolenzugriff wiederherstellen und in UART Konsole bestaetigen."); return; }
    const String source = r->hasArg("source") ? r->arg("source") : String("/dev/mtd7ro");
    if (!uartMonitor.startImageTransfer(source, 0U)) { r->send(409, "text/plain; charset=utf-8", uartMonitor.imageStatusJson()); return; }
    r->send(200, "application/json; charset=utf-8", uartMonitor.imageStatusJson());
  });
