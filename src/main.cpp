...
static const char* FW_BUILD_VERSION = "v0.21";
...
static String uartImageTransferPage() {
  ...
  h += "<p class='small'>Verifiziertes, blockweises Backup eines BX3-MTD-Devices oder einer vorbereiteten Snapshot-Datei ueber die serielle Shell. v0.21 erlaubt sichere Snapshot-Dateiquellen und verhindert, dass die Live-Statusabfrage Eingaben im Quellenfeld ueberschreibt. CRC32 (POSIX cksum) prueft jeden Block; die Gesamt-SHA256-Pruefung bleibt erhalten.</p>";
  ...
  h += uartNav(); h += "<a class='btn' href='/'>Zurueck</a>";
  h += R"rawliteral(<script>
let chunks=[],busy=false,pendingBlock=-1,pendingBytes=null,downloadUrl=null,startedAtBlock=0,lastStatus=null,imgSourceDirty=false;const imgSource=document.getElementById('img_source');imgSource.addEventListener('input',()=>{imgSourceDirty=true;});imgSource.addEventListener('focus',()=>{imgSourceDirty=true;});
...
async function startImage(){resetBrowserImage();startedAtBlock=Number(document.getElementById('img_start').value)||0;const src=document.getElementById('img_source').value.trim();document.getElementById('img_source').value=src;imgSourceDirty=false;const r=await post('/uart/image/start','source='+encodeURIComponent(src)+'&start='+startedAtBlock);if(!r.ok){document.getElementById('img_status').textContent=await r.text();return;}document.getElementById('img_note').textContent='Transfer gestartet.';pollImage();}
async function restartImage(){resetBrowserImage();startedAtBlock=0;document.getElementById('img_start').value='0';const src=((lastStatus&&lastStatus.source)||document.getElementById('img_source').value).trim();document.getElementById('img_source').value=src;imgSourceDirty=false;const r=await post('/uart/image/restart','source='+encodeURIComponent(src));if(!r.ok){document.getElementById('img_note').textContent='Neustart gesperrt: '+await r.text();return;}document.getElementById('img_note').textContent='Transfer nach Konsolenbestaetigung neu gestartet.';pollImage();}
